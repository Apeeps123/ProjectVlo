...
router.get('/', (req, res) => {
  connection.query('SELECT * FROM skin', (err, rows) => {
    ...
    return res.status(200).json({ status: true, message: 'Data Skin', data: rows });
  });
});

router.post('/store', upload.single('gambarSkin'), [
  body('skin').notEmpty(),
], (req, res) => {
...
router.patch('/update/:id', upload.single('gambarSkin'), (req, res) => {
  ...
  connection.query(`SELECT * FROM skin WHERE id = ?`, [id], (err, rows) => {
    ...
    if (gambarSkinLama && gambarSkin) {
      const pathGambarSkinLama = path.join(__dirname, '../public/images', gambarSkinLama);
    ...
    connection.query('UPDATE skin SET ? WHERE id = ?', [data, id], (err, rows) => {
      if (err) {
        return res.status(500).json({
          status: false,
          message: 'Server Error',
