...
router.get('/', (req, res) => {
  connection.query('SELECT * FROM roles', (err, rows) => {
    ...
    return res.status(200).json({ status: true, message: 'Data Roles', data: rows });
  });
});

router.post('/store', upload.single('icon'), [
  body('role').notEmpty(),
], (req, res) => {
...
router.patch('/update/:id', upload.single('icon'), (req, res) => {
  ...
  connection.query(`SELECT * FROM roles WHERE id = ?`, [id], (err, rows) => {
    ...
    connection.query('UPDATE roles SET ? WHERE id = ?', [data, id], (err, rows) => {
      if (err) {
        return res.status(500).json({
          status: false,
          message: 'Server Error',
...
router.delete('/delete/:id', (req, res) => {
  ...
  connection.query('DELETE FROM roles WHERE id = ?', [id], (err, rows) => {
    if (err) {
      return res.status(500).json({
        status: false,
        message: 'Server Error',
